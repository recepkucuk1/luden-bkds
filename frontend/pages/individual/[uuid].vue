...
<template>
  <div class="min-h-screen bg-white pb-8">
    ...
    <header class="bg-white border-b border-gray-200 pt-[env(safe-area-inset-top)]">
      <div class="px-4 py-3 flex items-center gap-3">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-[10px] uppercase tracking-wider text-gray-500">Birey</p>
    ...
    <!-- Error -->
    <div v-else-if="error" class="mx-4 mt-4 p-3 rounded-xl bg-red-50 text-red-700 text-sm">
      Yüklenemedi.
    ...
    <div v-else-if="data">
      ...
      <section class="mt-4 px-4">
        <button
          class="w-full text-left p-3 rounded-xl bg-gray-50 active:bg-gray-100 flex items-center justify-between"
        ...
        >
          <div
            v-if="showDetails"
            class="mt-2 p-3 rounded-xl bg-gray-50 grid grid-cols-2 gap-2 text-xs"
          >
            <div>
              <p class="text-gray-500">TC</p>
              <p class="text-gray-900 font-medium">
                {{ data.individual.identity_number }}
              </p>
            </div>
            <div>
              <p class="text-gray-500">Cinsiyet</p>
              <p class="text-gray-900 font-medium">{{ data.individual.gender }}</p>
            </div>
            <div v-if="data.individual.birth_date">
              <p class="text-gray-500">Doğum</p>
              <p class="text-gray-900 font-medium">{{ data.individual.birth_date }}</p>
            </div>
            <div v-if="data.individual.disability_code">
              <p class="text-gray-500">Engel kodu</p>
              <p class="text-gray-900 font-medium">{{ data.individual.disability_code }}</p>
            </div>
            <div>
              <p class="text-gray-500">Tip</p>
              <p class="text-gray-900 font-medium">
      ...
      <section class="mt-5">
        ...
        <div
          ...
          class="mx-4 p-4 bg-gray-50 rounded-xl text-center text-sm text-gray-500"
        >
          Bugün hareket yok
        </div>
        ...
        <div v-else class="px-4 space-y-1.5">
          <div
            ...
            :class="
              act.activity_type === 'entry' ? 'bg-brand-50' : 'bg-gray-50'
            "
          >
            <img
              v-if="act.roi_url"
            ...
            <div class="flex-1 min-w-0">
              <p
                ...
                :class="
                  act.activity_type === 'entry' ? 'text-brand' : 'text-gray-700'
                "
              >
                {{ act.activity_type === 'entry' ? 'Giriş' : 'Çıkış' }}
                <span v-if="act.is_matched_manually" class="ml-1 text-[10px] text-amber-700">
                  (manuel)
                </span>
              </p>
            ...
            <div
              class="text-lg"
              :class="act.activity_type === 'entry' ? 'text-brand' : 'text-gray-400'"
            >
