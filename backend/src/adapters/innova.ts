...
export class InnovaBryAdapter {
  ...
  /**
   * Bugünün giriş-çıkış özeti (kim girdi, ilk giriş + son çıkış).
  ...
  async getTodayActivitySummary(opts?: {
    ...
    const { startTime, endTime } = computeTodayUtcRange();
  ...
  /**
   * Tek bireyin tüm aktivitelerini çeker (zaman aralığında).
   * "Şu an içeride mi?" sorusunu cevaplamak için kullanılır.
   */
...
/**
 * Türkiye saati ile "bugün"ün UTC karşılığını hesaplar.
 * INNOVA tarih aralığını UTC olarak istiyor (T21:00:00Z = TR 00:00).
 */
function computeTodayUtcRange(): { startTime: string; endTime: string } {
  const now = new Date();
  // TR ofseti: Date'in toLocaleString'i ile TR günü belirleyelim
